...
class TextSplitting:
    ...
    def mineru_recursive_splitter(self, file_path):
        docs = DocumentLoading().process_pdf(file_path)
    ...
    def pypdf_recursive_splitter(self, file_path):
    ...
    def unstructured_recursive_splitter(self, file_path):
        docs = DocumentLoading().unstructured_loader(file_path)
        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=300,
            chunk_overlap=20,
            length_function=len,
    ...
    # openai api needed
    def pypdf_semantic_splitter(self, file_path):
        docs = DocumentLoading().pypdf_loader(file_path)
