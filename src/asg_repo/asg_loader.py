'''
1. some spaces not expected, like RELATED to R ELATED
2. for categorization, only abstracts and introductions are used
3. combine the two loader functions into one (unstructured_loader and unsructured_loader_old)
4. improvement can be made for the extraction of abstract and introduction by enhancing the regex pattern
5. some more details on clean funcitons not yet implemented
'''

import re
import json
import spacy
from langchain_community.document_loaders import UnstructuredPDFLoader, PyPDFLoader, PDFMinerLoader, PyPDFDirectoryLoader
...
class DocumentLoading:
    ...
    def unstructured_loader_extract(self, file_path):
        '''
        Double '\\n' between sections \n
        Extract information using unstructured pdf loader
        '''
    ...
    def unstructured_loader_old(self, file_path):
        '''
    ...
    def unstructured_loader(self, file_path):
        '''
        Extract abstract and introduction using unstructured pdf loader \n
        Better extraction of abstract and introduction compared with unstructured_loader_old
        '''
        loader = UnstructuredPDFLoader(file_path)
        documents = loader.load()
        ...
        if abstract_match:
            ...
            temp_text += content[abstract_match.start():]  # 从 "Abstract" 位置开始

        if start_extracting:
            related_work_match = re.search(r'\bRELATED\s*WORK\b', content, re.IGNORECASE)
            if related_work_match:
                temp_text += content[:related_work_match.start()]  # 截止到 "Related Work" 位置
            else:
                related_work_match_space = re.search(r'\bR\s*E\s*L\s*A\s*T\s*E\s*D\s* \s*W\s*O\s*R\s*K\b', content, re.IGNORECASE)
                if related_work_match_space:
                    temp_text += content[:related_work_match_space.start()]  # 处理包含空格的情况，如 "R ELATED WORK"
                else:
                    temp_text += content

        extracted_text = temp_text
        extracted_text = self.clean_hyphenation_unstructured(extracted_text)
    ...
    def pypdf_directory_loader(self, file_path):
        '''
        Need to test the use \n
        Work for directory of pdf files
        '''
...
if __name__ == "__main__":
    ...
    # asg_loader.unstructured_loader_extract("./Test1.pdf")
    # asg_loader.unstructured_loader_extract("./Test2.pdf")
    # asg_loader.unstructured_loader_extract("./Test3.pdf")
    # asg_loader.unstructured_loader_extract("./Test6.pdf")
    # asg_loader.unstructured_loader_extract("./Test7.pdf")
    # asg_loader.unstructured_loader_extract("./Test8.pdf")
